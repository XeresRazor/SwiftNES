...
//  SwiftNES
//
//  Created by David Green on 5/27/15.
...
var fontMask = initFontMask()

func initFontMask() -> AlphaImage {
    let im: RGBAImage? = RGBAImage(pngData:fontData)
    
    if im  == nil {
        fatalError("Unable to load font image")
    }
    let size = im!.Rect.Size()
    let mask = AlphaImage(r:im!.Bounds())
    for var y = 0; y < size.Y; y++ {
        for var x = 0; x < size.X; x++ {
            let color = im!.At(x, y)
            if color.RGBA().r > UInt32(0) {
                mask.Set(x, y, c: OpaqueColor)
            }
        }
    }
    return mask
...
func bitmapFromImage(image: Image) -> NSBitmapImageRep {
    if let im = image as? RGBAImage {
        ...
        return bitmap!
    } else if let im = image as? AlphaImage {
        let w = im.Rect.Size().X
        let h = im.Rect.Size().Y
...
func drawCharacter(dest: Image, x: Int, y: Int, character: CChar, color: Color) {
    let ch = UInt8(character)
    if ch < 32 || ch > 128 {
        return
    }
